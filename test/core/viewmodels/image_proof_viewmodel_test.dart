...
void main() {
  test(
    ...
    () async {
      ...
      when(
        imageProcessingService.processImage(original, transformations),
      ...
      when(
        imageProcessingService.optimizeForProofGeneration(original),
      ).thenAnswer((_) async => original);
      when(
        imageProcessingService.optimizeForProofGeneration(edited),
      ).thenAnswer((_) async => edited);
      when(imageProcessingService.getImageFormat(edited)).thenReturn('jpeg');
...
class MockImageProofService extends Mock implements ImageProofService {
  ...
  Future<ImageProof> generateProof({
    ...
    return super.noSuchMethod(
          Invocation.method(#generateProof, const [], <Symbol, dynamic>{
...
class MockImageProcessingService extends Mock
  ...
  Future<Uint8List> processImage(
    ...
    return super.noSuchMethod(
          Invocation.method(#processImage, <Object?>[
            imageData,
            transformations,
          ]),
  ...
  String getImageFormat(Uint8List imageData) {
    return super.noSuchMethod(
          Invocation.method(#getImageFormat, <Object?>[imageData]),
          returnValue: 'jpeg',
        )
        as String;
  }
